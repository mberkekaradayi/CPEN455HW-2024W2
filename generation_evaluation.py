...
def my_sample(model, gen_data_dir, sample_batch_size = 25, obs = (3,32,32), sample_op = sample_op):
    ...
    for label in my_bidict:
        ...
        class_names = [label] * sample_batch_size

        sample_c = sample(model, sample_batch_size, obs, sample_op, labels=class_names)
        sample_c = rescaling_inv(sample_c)
        save_images(sample_c, os.path.join(gen_data_dir), label=label)
